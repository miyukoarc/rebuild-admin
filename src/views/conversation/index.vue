...
export default {
  ...
  computed: {
    ...mapState({
      ...
      currentUnreadCount: state => state['im/conversation'].currentConversation.unreadCount,
      currentMessageList: state => state['im/conversation'].currentMessageList,
      isCompleted: state => state['im/conversation'].isCompleted,
      conversationID: state => state['im/conversation'].currentConversation.conversationID
    }),
    ...mapGetters({
  ...
  methods: {
    ...
    showProfile() {
      ...
      this.$store.commit('component/TOGGLE_CONVERSATIONPROFILE',this.showCurrentProfile)
