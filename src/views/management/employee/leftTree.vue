<template>
  <el-container style="width:100%" id="emp-tree">
    ...
    <el-tree :data="datalist" node-key="uuid" :expand-on-click-node="false" block
      :default-expanded-keys='[datalist[0].uuid]' style="width:100%" highlight-current @node-click="onNodeClick">
        ...
        <span class="custom-tree-node" slot-scope="{ node, data }">
          ...
          <el-tooltip placement="top" :content="data.name">
            <span class="tree-node-name" ref="nodeName">{{data.name}} ({{isEmptyObject(data.users)?0:data.users.length}})</span>
          </el-tooltip>
        </span>
    </el-tree>
  </el-container>
