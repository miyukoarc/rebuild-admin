...
const actions = {
  ...
  checkoutConversation(context, conversationID) {
    context.commit('im/group/resetCurrentMemberList', {}, {
      root: true
    })
    ...
    // 3. 获取会话信息
    return tim.getConversationProfile(conversationID).then(({
      data
    }) => {
      // 3.1 更新当前会话
      ...
      if (data.conversation.type === TIM.TYPES.CONV_GROUP) {
        return context.dispatch('im/group/getGroupMemberList', data.conversation.groupProfile.groupID,{root:true})
      }
