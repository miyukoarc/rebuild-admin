<template>
  <div v-if="dialling || calling || isDialled" class="call-container">
    ...
    <div v-if="dialling || calling" class="call">
      ...
      <div v-show="calling" id="local" :class="isLocalMain ? 'big' : 'small'" @click="changeMainVideo" />
      <div v-show="calling" name="remote" :class="isLocalMain ? 'small' : 'big'" @click="changeMainVideo" />
      <div v-show="calling" class="duration">
        {{ formatDurationStr }}
      </div>
      <div class="buttons">
        <div :class="isCamOn ? 'videoOn' : 'videoOff'" @click="videoHandler" />
        <div class="refuse" @click="leave" />
        <div :class="isMicOn ? 'micOn' : 'micOff'" @click="micHandler" />
      </div>
      <div v-show="maskShow" class="mask" :class="isLocalMain ? 'big' : 'small'" @click="changeMainVideo">
        <div>
...
export default {
  ...
  computed: {
    ...
    ...mapState({
      ...
      userSig: state => state['im/user'].userSig,
      videoRoom: state => state['im/video'].videoRoom,
      sdkAppID: state => state['im/user'].sdkAppID
    }),
    formatDurationStr() {
  ...
  methods: {
    ...
    changeState(state, boolean) {
      ...
      this.$store.commit('im/video/UPDATE_ISBUSY', stateList.some(item => this[item])) // 若stateList 中存在 true , isBusy 为 true
    },
    ...
    videoCall() { // 发起通话
      ...
      this.$store.commit('im/video/GENERATE_VIDEO_ROOM') // 初始化房间号
      const options = {
        userId: this.userID,
