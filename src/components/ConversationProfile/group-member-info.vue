<template>
  <div>
    <div>
      ...
      <el-popover v-show="showBan" v-model="popoverVisible" title="禁言">
        <el-input
          v-model="muteTime"
          placeholder="请输入禁言时间"
    ...
    <div>
      ...
      {{ member.nameCard || '暂无' }}
      <el-popover v-show="showEditNameCard" v-model="nameCardPopoverVisible" title="修改群名片">
        <el-input
          v-model="nameCard"
          placeholder="请输入群名片"
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      currentUserProfile: state => state['im/user'].currentUserProfile,
      current: state => state['im/setting'].current
    }),
    // 是否显示踢出群成员按钮
    showKickout() {
